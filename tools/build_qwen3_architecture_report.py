...
def _architecture_rows() -> str:
    rows = [
        ...
        ("Graph 生命周期", "启动期捕获；固定地址 staging；replay 后独立 stream metadata update；event + 双缓冲。",
         ...
         "torch.compile + ACL graph piecewise；本次启动捕获 35 个 size。"),
...
<div class="thesis-chain"><div><span>01 / CONTRACT</span><strong>同模型、同 KV、同精度</strong></div><div><span>02 / MEASURE</span><strong>20 次无 profiler 主测试</strong></div><div><span>03 / EXPLAIN</span><strong>1 prefill + 15 decode trace</strong></div><div><span>04 / GOVERN</span><strong>框架不变量 / 模型生成物</strong></div></div></header>

<section id="executive-summary"><div class="section-head"><span class="eyebrow">EXECUTIVE DECISION</span><h2>管理结论</h2><p>在 Qwen3-0.6B、单张 Ascend 910B1、BF16 greedy、等 14,464 KV tokens 的验收边界内，auto-infer 在稳态延迟、吞吐、启动、等容量内存与稳定性上全部第一。</p></div>
<div class="decision-grid"><article class="decision"><span class="evidence measured">实测</span><strong class="big">B16: 2,259.2 tok/s</strong><p>对 omni-npu 为 <strong>{relative["omni-npu"]["throughput_speedup"]:.2f}×</strong>，对 vllm-ascend 为 <strong>{relative["vllm-ascend"]["throughput_speedup"]:.2f}×</strong>。auto-infer 与 vllm-ascend 的 128-token digest 一致。</p><ul><li>Warm TTFT：{data["auto-infer"]["ttft_seconds"]["median"]*1000:.3f} ms</li><li>TPOT：{data["auto-infer"]["tpot_seconds"]*1000:.3f} ms</li><li>Throughput CV：{data["auto-infer"]["stability"]["throughput_cv"]*100:.3f}%</li></ul></article>
...
<section id="matched-benchmark"><div class="section-head"><span class="eyebrow">UNPROFILED · AUTHORITATIVE</span><h2>Matched benchmark</h2><p>Qwen3-0.6B；BF16；greedy / ignore-EOS；B1 latency；B16 throughput；128 output tokens；1 次 warm-up；20 次 measurement；每框架 14,464 usable KV tokens。以下 headline 只来自无 profiler 原始 JSON。</p></div>
...
<section id="profiling-deep-dive"><div class="section-head"><span class="eyebrow">RAW CHROME TRACE · DIRECTLY OPENABLE</span><h2>Qwen3 三框架 profiling</h2><p>每份 trace 捕获同一 B16、16-token generate：<strong>{phases["prefill_passes"]} 次 prefill</strong> + <strong>{phases["decode_passes"]} 次连续 decode</strong>。这是连续多步 decode，<strong>不是投机 MTP</strong>。三方均记录到 448 次 fused-attention host 调用，与 28 层 × 16 次 forward 相符。</p></div>
