...
def _runtime_call_stack_data(summary: dict) -> list[tuple[str, str, str, str, str]]:
    ...
    auto_prefill = summary["profiles"]["auto-infer"]["execution_phases"][
        "steps"][0]["duration_us"]
    rows = []
    for framework in FRAMEWORKS:
        profile = summary["profiles"][framework]
        ...
        prefill_comparison = (
            "baseline" if framework == "auto-infer"
            else f"{prefill_ms * 1000 / auto_prefill:.2f}× slower than auto-infer")
        rows.append((
            framework,
            f"{prefill_ms:.2f} ms ({prefill_comparison})",
...
def _causal_data(summary: dict, manifest: dict) -> list[tuple[str, str, str, str]]:
    ...
    return [
        ...
        ("实测", "Profiler window",
         ...
         "三份 raw Chrome Trace"),
        ("实测", "Startup",
         f'{data["auto-infer"]["load_seconds"]:.3f} s vs {data["omni-npu"]["load_seconds"]:.3f} s vs {data["vllm-ascend"]["load_seconds"]:.3f} s；auto-infer 与 vllm-ascend 都覆盖到 256-token graph family，Omni 覆盖到 512；启动差距不能只归因于 gear 数。',
         "headline benchmark + captured framework configuration logs"),
    ]
...
def build_report(summary: dict, manifest: dict) -> str:
    ...
    headline_workload = data["auto-infer"]["manifest"]
    profile_workload = manifest["workload"]
    prompt_token_count = manifest["provenance"]["model"]["prompt_token_count"]
    flattened_prefill_tokens = (
        profile_workload["batch_size"] * prompt_token_count)
    auto_peak_gib = data["auto-infer"]["peak_allocated_gib"]
    vllm_peak_gib = data["vllm-ascend"]["peak_allocated_gib"]
    omni_peak_gib = data["omni-npu"]["peak_allocated_gib"]
    benchmark_evidence_dir = Path(next(
        iter(manifest["benchmark_sources"].values()))).parent
    ...
    prefill_ms = {
        framework: summary["profiles"][framework]["execution_phases"][
            "steps"][0]["duration_us"] / 1000
        for framework in FRAMEWORKS
    }
    auto_prefill_omni_ratio = prefill_ms["omni-npu"] / prefill_ms["auto-infer"]
    auto_prefill_vllm_ratio = prefill_ms["vllm-ascend"] / prefill_ms["auto-infer"]
...
<div class="callout"><strong>Prefill 执行模式门禁：</strong>B{profile_workload["batch_size"]} × {prompt_token_count}-token prompt 形成 {flattened_prefill_tokens} 个 flattened query tokens。prefill 使用独立的 {profile_workload["max_prefill_tokens"]}-token graph cap，不与 decode request gear 共用上限；采集器与发布 analyzer 都强制要求 <code>prefill-graph=1</code>、<code>eager=0</code>、<code>online capture=0</code>，否则拒绝发布 trace。</div>
...
<figure class="diagram"><img src="../figures/qwen3-trace-call-stack-comparison.svg" alt="Trace-derived Qwen3 prefill and decode runtime call stacks for all three frameworks"><figcaption>同一行使用公共毫秒比例尺；bar 的位置、宽度和嵌套深度都来自原始 trace。auto-infer prefill 为 {prefill_ms["auto-infer"]:.2f} ms，相对 omni-npu 快 {auto_prefill_omni_ratio:.2f}×、相对 vllm-ascend 快 {auto_prefill_vllm_ratio:.2f}×；decode 中位数为 {auto_decode_ms:.2f} ms，相对两者快 {omni_decode_ratio:.2f}× / {vllm_decode_ratio:.2f}×。</figcaption></figure>
...
<p class="callout warning"><strong>因果边界：</strong>trace 证明本次 auto-infer 的 prefill 与 decode host range 都更短，且 prefill 实际经过 <code>GraphPagedRunner._prefill_graph_submit</code>；这不等于证明所有未插桩函数都不存在，也不能只凭“层数更少”断言全部加速因果。<code>record_function</code> 对每个边界也有小幅且不同数量的扰动，因此排名仍以无 profiler headline 为准。</p></section>
...
def build_markdown_report(summary: dict, manifest: dict) -> str:
    ...
    prompt_token_count = manifest["provenance"]["model"]["prompt_token_count"]
    flattened_prefill_tokens = workload["batch_size"] * prompt_token_count
    auto_peak_gib = data["auto-infer"]["peak_allocated_gib"]
    vllm_peak_gib = data["vllm-ascend"]["peak_allocated_gib"]
    omni_peak_gib = data["omni-npu"]["peak_allocated_gib"]
...
在 `{Path(headline['model']).name}`、单张 `{driver['soc']}`、`{headline['dtype']}` greedy、每框架 `{headline['usable_kv_tokens']:,}` usable KV tokens 的验收边界内，auto-infer 的稳态延迟、吞吐、启动与稳定性均为本次第一。内存是明确代价：peak torch allocation 为 {auto_peak_gib:.2f} GiB，高于 vllm-ascend 的 {vllm_peak_gib:.2f} GiB，但低于 omni-npu 的 {omni_peak_gib:.2f} GiB。
...
**Prefill 执行模式门禁：**B{workload['batch_size']} × {prompt_token_count}-token prompt 形成 {flattened_prefill_tokens} 个 flattened query tokens。prefill 使用独立的 {workload['max_prefill_tokens']}-token graph cap，不与 decode request gear 共用上限；采集器与发布 analyzer 都强制要求 `prefill-graph=1`、`eager=0`、`online capture=0`，否则拒绝发布 trace。
